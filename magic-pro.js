...
(function(){
  ...
  /* ---------- 1. Cricket-ball cursor (desktop only) — spins as it moves ---------- */
  if(!touch){
    // hide every native cursor so the ball is the only pointer on screen
    var cs=document.createElement("style");cs.textContent="*{cursor:none!important}";document.head.appendChild(cs);
    var cur=document.createElement("div");
    cur.style.cssText="position:fixed;top:0;left:0;width:26px;height:26px;z-index:130;pointer-events:none;transform:translate(-50%,-50%);transition:width .18s,height .18s,filter .18s;filter:drop-shadow(0 3px 6px rgba(0,0,0,.55))";
    var ball=document.createElement("div");
    ball.style.cssText="width:100%;height:100%;will-change:transform";
    ball.innerHTML='<svg viewBox="0 0 26 26" width="100%" height="100%" style="display:block">'+
      '<defs><radialGradient id="cbG" cx="36%" cy="30%" r="78%">'+
        '<stop offset="0" stop-color="#ff8a8a"/><stop offset="52%" stop-color="#d21f2f"/><stop offset="100%" stop-color="#6f0a14"/>'+
      '</radialGradient></defs>'+
      '<circle cx="13" cy="13" r="12" fill="url(#cbG)" stroke="#4d0710" stroke-width="1"/>'+
      '<path d="M8 2.6 Q13 13 8 23.4" fill="none" stroke="#ffe9b0" stroke-width="1.1" stroke-dasharray="1.6 1.9" opacity=".95"/>'+
      '<path d="M18 2.6 Q13 13 18 23.4" fill="none" stroke="#ffe9b0" stroke-width="1.1" stroke-dasharray="1.6 1.9" opacity=".95"/>'+
      '<ellipse cx="9" cy="8" rx="3.3" ry="2.1" fill="rgba(255,255,255,.30)"/>'+
    '</svg>';
    cur.appendChild(ball);
    document.body.appendChild(cur);
    var cx=innerWidth/2,cy=innerHeight/2,tx=cx,ty=cy,rot=0;
    addEventListener("mousemove",function(e){tx=e.clientX;ty=e.clientY;});
    (function follow(){
      var dx=tx-cx,dy=ty-cy;
      cx+=dx*0.18;cy+=dy*0.18;
      rot+=dx*0.5;                 // spin proportional to horizontal speed
      cur.style.left=cx+"px";cur.style.top=cy+"px";
      ball.style.transform="rotate("+rot+"deg)";
      requestAnimationFrame(follow);
    })();
    var hot="a,button,.card,.chip,.fbtn,input,.theme-toggle,.music-toggle,.lbtab,.lbrow,.pcard,.soc,.mu-top";
    document.addEventListener("mouseover",function(e){
      if(e.target.closest(hot)){cur.style.width="42px";cur.style.height="42px";cur.style.filter="drop-shadow(0 0 12px rgba(255,120,120,.85))";}
    });
    document.addEventListener("mouseout",function(e){
      if(e.target.closest(hot)){cur.style.width="26px";cur.style.height="26px";cur.style.filter="drop-shadow(0 3px 6px rgba(0,0,0,.55))";}
    });
  }
  ...
  pill.style.cssText="position:fixed;left:22px;bottom:22px;z-index:80;display:flex;align-items:center;gap:10px;padding:10px 16px;border-radius:999px;background:rgba(11,26,64,.7);border:1px solid rgba(241,193,75,.3);backdrop-filter:blur(10px);font-size:13px;font-weight:600;color:#dbe6ff;box-shadow:0 10px 26px rgba(0,0,0,.4)";
  pill.innerHTML='<span style="width:9px;height:9px;border-radius:50%;background:#5b8dff;box-shadow:0 0 10px #5b8dff;animation:lp 1.6s infinite"></span><span>🏙️ Lucknow</span><span id="clk" style="color:#ffe6a0;font-variant-numeric:tabular-nums"></span>';
  ...
  function fireworks(){
    ...
    var ps=[],cols=["#f2c250","#ffe6a0","#5b8dff","#e23150","#fff","#3f72e0"];
